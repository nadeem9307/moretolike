...
let DropshipComponent = class DropshipComponent {
    constructor(imgBaseUrl, router, toastr, _categoryService, _dropshipService) {
    ...
    ngOnInit() {
        ...
        this._dropshipService.getAll().subscribe(data => {
            // console.log(data);
    ...
    search() {
        this._dropshipService.searching({ search_value: this.searchValue, dropdown_value: this.dropDownValue }).subscribe(data => {
            console.log(data);
            this.dropshipData = data.success;
        });
    }
    dropship_delete(del_id) {
        console.log(del_id);
        let value = confirm('Are you want to delete this dropship ?');
        if (value) {
            this._dropshipService.del_dropship(del_id).subscribe(data => {
                this.dropshipData = data.data;
            });
        }
    }
    changeStatus(dropship) {
        if (dropship.dropship_status == 'Active') {
            dropship.dropship_status = 'Inactive';
            this._dropshipService.updateStatus(dropship.id).subscribe(data => {
    ...
    orderFilter() {
        ...
        this._dropshipService.filter({ search_value: this.searchValue, dropdown_value: this.dropDownValue, loggedUserId: loggedUserId.success.id, loggedUserRole: loggedUserId.success.role }).subscribe(data => {
            this.productData = data.success;
        });
    }
    checkAll(productData) {
        let loggedUserId = JSON.parse(localStorage.getItem('currentUser'));
    }
    isAnswerProvided(event, product) {
...
let DropshipModule = class DropshipModule {
};
...
//# sourceMappingURL=pages-default-dropship-dropship-module-es2015.js.map