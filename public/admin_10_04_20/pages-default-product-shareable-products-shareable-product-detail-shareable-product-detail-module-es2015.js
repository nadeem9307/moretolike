...
let ShareableProductDetailComponent = class ShareableProductDetailComponent {
    constructor(route, _dataService, _productService) {
        this.route = route;
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe(params => {
            ...
            this._productService.getById(params.id).subscribe(data => {
                this.productDetails = data.success[0];
                loggedVendorId = JSON.parse(localStorage.getItem('currentUser'));
                sharedVendor = this.productDetails.shared_vendor;
    ...
    addProductInStore() {
        let current_logged_user_id = JSON.parse(localStorage.getItem('currentUser'));
        ...
        if (current_logged_user_id) {
            this._productService.addShareableProduct({ vendor_id: current_logged_user_id, product_id: this.productId }).subscribe(data => {
                if (data.success == 1) {
                    this.showAddBtn = 1;
                    this._productService.getNotification(current_logged_user_id).subscribe(datas => {
                        this._dataService.setNotification(datas.success.length);
                        console.log(datas.success.length);
                    });
                    this._dataService.getNotification().subscribe(not_data => {
...
let ShareableProductDetailModule = class ShareableProductDetailModule {
};
...
//# sourceMappingURL=pages-default-product-shareable-products-shareable-product-detail-shareable-product-detail-module-es2015.js.map