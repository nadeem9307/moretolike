...
let BrandListComponent = class BrandListComponent {
    constructor(router, imgBaseUrl, toastr, _brandService, _dataService, formBuilder) {
        this.router = router;
    ...
    ngOnInit() {
        ...
        this._brandService.getAll().subscribe(data => {
    ...
    onSubmit() {
        ...
        if (this.form.valid && this.brandImgUrl) {
            ...
            this._brandService.getByName(input.form_data.name).subscribe(searchData => {
                if (searchData.success) {
                    this.toastr.error('Brand Already Exist', 'Error Message');
                }
                else {
                    this._brandService.createBrand(input).subscribe(data => {
                        this.brandData = data.data;
                        this.toastr.success('Brand Added Successfully', 'Success Message');
                        this.reset();
                    });
                }
            });
    ...
    brand_delete(del_id) {
        ...
        if (value) {
            this._brandService.brandDelete(del_id).subscribe(data => {
                this.brandData = data.data;
            });
        }
    }
    changeBrandStatus(brand) {
        console.log(brand);
        if (brand.status == 'Active') {
            brand.status = 'Inactive';
            this._brandService.updateStatus(brand.id).subscribe(data => {
            });
        }
        else {
            brand.status = 'Active';
    ...
    search() {
        this._brandService.searching({ search_value: this.searchValue, dropdown_value: this.dropDownValue }).subscribe(data => {
            this.brandData = data.success;
        });
    }
};
...
//# sourceMappingURL=pages-default-brand-brand-list-brand-list-module-es2015.js.map